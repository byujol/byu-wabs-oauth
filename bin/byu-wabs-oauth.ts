...
function byuOauth (clientId: string, clientSecret: string, wellKnownUrl: string) : ByuOauthFactory {
    ...
    const refreshTokens = Promise.coroutine(function *(access_token, refresh_token) {
        if (!refresh_token) throw Error("Refresh token required to refresh tokens");
        const oauth2Handle = yield getOauth2Handle(clientId, clientSecret, wellKnownUrl);
...
function processToken(wellKnownURL, clientToken) : Promise<Token> {
    const token = {
    ...
    if (!clientToken.id_token) return Promise.resolve(token);
